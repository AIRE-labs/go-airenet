...
import (
	...
	"github.com/AIRE-labs/go-airenet/cmd/aire/launcher/metrics"
	"github.com/AIRE-labs/go-airenet/cmd/aire/launcher/tracing"
	"github.com/AIRE-labs/go-airenet/debug"
	"github.com/AIRE-labs/go-airenet/flags"
	"github.com/AIRE-labs/go-airenet/gossip"
	"github.com/AIRE-labs/go-airenet/integration"
	"github.com/AIRE-labs/go-airenet/utils/errlock"
	"github.com/AIRE-labs/go-airenet/valkeystore"
	_ "github.com/AIRE-labs/go-airenet/version"
)

const (
	// clientIdentifier to advertise over the network.
	clientIdentifier = "go-airenet"
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-airenet command line interface")
...
func initFlags() {
	...
	nodeFlags = append(nodeFlags, aireFlags...)
...
// aire is the main entry point into the system if no special subcommand is ran.
...
func lachesisMain(ctx *cli.Context) error {
	...
	genesisPath := getAIREGenesis(ctx)
...
func makeNode(ctx *cli.Context, cfg *config, genesis integration.InputGenesis) (*node.Node, *gossip.Service, func()) {
	...
	valPubkey := cfg.AIRE.Emitter.Validator.PubKey
	if key := getFakeValidatorKey(ctx); key != nil && cfg.AIRE.Emitter.Validator.ID != 0 {
	...
	svc, err := gossip.NewService(stack, cfg.AIRE, gdb, signer, blockProc, engine, dagIndex)
	if err != nil {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	// Create a client to interact with local aire node.
	...
	if ctx.GlobalBool(utils.ExitWhenSyncedFlag.Name) {
		go func() {
			for first := true; ; first = false {
				// Call aire_syncing until it returns false
				time.Sleep(5 * time.Second)

				var syncing bool
				err := rpcClient.CallContext(context.TODO(), &syncing, "aire_syncing")
				if err != nil {
					continue
				}
				if !syncing {
