...
var (
	...
	AllowedAIREGenesisHashes = map[uint64]hash.Hash{
		aire.MainNetworkID: hash.HexToHash("0x4a53c5445584b3bfc20dbfb2ec18ae20037c716f3ba2d9e1da768a9deca17cb4"),
		aire.TestNetworkID: hash.HexToHash("0xc4a5fc96e575a16a9a0c7349d44dc4d0f602a54e0a8543360c2fee4c3937b49e"),
	}
)
...
func (c *config) AppConfigs() integration.Configs {
	return integration.Configs{
		AIRE:          c.AIRE,
		AIREStore:     c.AIREStore,
		Lachesis:       c.Lachesis,
		LachesisStore:  c.LachesisStore,
		VectorClock:    c.VectorClock,
		AllowedGenesis: AllowedAIREGenesisHashes,
	}
}
...
func getAIREGenesis(ctx *cli.Context) integration.InputGenesis {
...
func mayMakeAllConfigs(ctx *cli.Context) (*config, error) {
	...
	cfg := config{
		...
		AIRE:         gossip.DefaultConfig(cacheRatio),
		AIREStore:    gossip.DefaultStoreConfig(cacheRatio),
	...
	if ctx.GlobalIsSet(FakeNetFlag.Name) {
		...
		cfg.AIRE = gossip.FakeConfig(num, cacheRatio)
	}
	...
	var err error
	cfg.AIRE, err = gossipConfigWithFlags(ctx, cfg.AIRE)
	if err != nil {
		return nil, err
	}
	cfg.AIREStore, err = gossipStoreConfigWithFlags(ctx, cfg.AIREStore)
	if err != nil {
		return nil, err
	}
	cfg.Node = nodeConfigWithFlags(ctx, cfg.Node)
	if cfg.AIRE.Emitter.Validator.ID != 0 && len(cfg.AIRE.Emitter.PrevEmittedEventFile.Path) == 0 {
		cfg.AIRE.Emitter.PrevEmittedEventFile.Path = cfg.Node.ResolvePath(path.Join("emitter", fmt.Sprintf("last-%d", cfg.AIRE.Emitter.Validator.ID)))
	}

	if err := cfg.AIRE.Validate(); err != nil {
		return nil, err
	}
