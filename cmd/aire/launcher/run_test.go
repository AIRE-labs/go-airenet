...
import (
	...
	"github.com/AIRE-labs/go-airenet/cmd/cmdtest"
)

func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "aire-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "aire-test" in exec().
	reexec.Register("aire-test", func() {
...
func exec(t *testing.T, args ...string) *testcli {
	...
	// Boot "aire". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("aire-test", args...)
